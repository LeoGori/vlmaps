...
class OVSeg(MaskFormer):
    ...
    def original_forward(self, batched_inputs):
        """
        Args:
    ...
    def forward(self, batched_inputs, class_names):
        """
        Args:
            batched_inputs: a list, batched outputs of :class:`DatasetMapper`.
                Each item in the list contains the inputs for one image.
        ...
        if self.training:
            ...
            for mask_cls_result, mask_pred_result, input_per_image, image_size in zip(
                ...
                r, regions, bin_mask, image_feat = self.semantic_inference(
                    mask_cls_result, mask_pred_result, image, class_names
                )
                bin_masks.append(torch.tensor(bin_mask))
                image_feats.append(image_feat)

                height = input_per_image.get("height", image_size[0])
                width = input_per_image.get("width", image_size[1])
                r = sem_seg_postprocess(r, image_size, height, width)
                processed_results.append({"sem_seg": r.cpu()})
                ...
                if self.panoptic_on:
                    ...
                    processed_results[-1]["panoptic_seg"] = panoptic_r.cpu()

            return processed_results, (torch.stack(image_feats), torch.stack(bin_masks))
    ...
    def get_image_embeddings(self, batched_inputs):
        ...
        for mask_pred_result, input_per_image, image_size in zip(
            ...
            bin_mask, image_feat = self.get_inference(
                mask_pred_result, image
            )

            bin_masks.append(torch.tensor(bin_mask))
            image_feats.append(image_feat)

        pred_logits = torch.stack(image_feats).squeeze().cpu()
        pred_masks = torch.stack(bin_masks).squeeze().cpu()

        pred_masks = sem_seg_postprocess(
                    pred_masks.float(), pred_masks.shape[1:], images.tensor.shape[-2], images.tensor.shape[-1]
                )
        
        featured_image = torch.zeros(pred_masks.shape[1], pred_masks.shape[2], pred_logits.shape[1])

        for (pred_logit, pred_mask) in zip(pred_logits, pred_masks):
            
            pred_mask = pred_mask.unsqueeze(-1).expand(featured_image.shape)
            pred_logit = pred_logit.view(1, 1, -1)
            region_feats = pred_logit * pred_mask.float()
    ...
    def get_inference(self, mask_pred, image):
        mask_pred = mask_pred.sigmoid()

        regions = None
        if self.clip_ensemble:
            regions, valid_flag, image_feats = self.clip_adapter.get_image_embeddings(
    ...
    def semantic_inference(self, mask_cls, mask_pred, image, class_names):
        ...
        regions = None
        if self.clip_ensemble:
            clip_cls, regions, valid_flag, image_feats = self.clip_adapter(
        ...
        return semseg, regions, bin_mask, image_feats
...
class OVSegDEMO(MaskFormer):
    ...
    def forward(self, batched_inputs):
        ...
        for mask_cls_result, mask_pred_result, input_per_image, image_size in zip(
            ...
            r, regions, bin_mask, image_feat = self.demo_inference(mask_cls_result, mask_pred_result, image, class_names)
            bin_masks.append(torch.tensor(bin_mask))
            image_feats.append(image_feat)
    ...
    def get_image_embeddings(self, batched_inputs):
        ...
        for mask_pred_result, input_per_image, image_size in zip(
            ...
            bin_mask, image_feat = self.get_inference(mask_pred_result, image)
            bin_masks.append(torch.tensor(bin_mask))
            image_feats.append(image_feat)
