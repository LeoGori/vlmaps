...
class CATSeg(nn.Module):
    ...
    def get_masks(self, downsampled_batched_image_feats, labels):
        ...
        downscaled_res3 = rearrange(downsampled_batched_image_feats, "B (H W) C -> B C H W", H=24)
        downscaled_res4 = rearrange(self.layers[0][1:, :, :], "(H W) B C -> B C H W", H=24)
        downscaled_res5 = rearrange(self.layers[1][1:, :, :], "(H W) B C -> B C H W", H=24)
        downscaled_res4 = self.upsample1(downscaled_res4)
        downscaled_res5 = self.upsample2(downscaled_res5)
        ...
        downscaled_outputs = self.sem_seg_head(downsampled_batched_image_feats, downscaled_image_features, omit_first_feats=False)

        if self.training:
            targets = torch.stack([x["sem_seg"].to(self.device) for x in downsampled_batched_image_feats], dim=0)
            outputs = F.interpolate(outputs, size=(targets.shape[-2], targets.shape[-1]), mode="bilinear", align_corners=False)
            ...
            downscaled_outputs = downscaled_outputs.sigmoid()
            image_size = downscaled_res3.shape[1:3]
            height = downsampled_batched_image_feats[0].get("height", image_size[0])
            width = downsampled_batched_image_feats[0].get("width", image_size[1])

            downscaled_output = sem_seg_postprocess(downscaled_outputs[0], image_size, height, width)

            processed_results = [{'sem_seg': downscaled_output}]
