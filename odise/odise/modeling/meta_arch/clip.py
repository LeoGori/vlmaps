...
class MaskCLIP(ClipAdapter):
    ...
    def encode_image_with_mask(self, image, mask):
        ...
        # print(f"mask token attention mask: {mask_token_attn_mask.shape}")

        # msk_tkn_attn_mask = {"mask_token_attn_mask": mask_token_attn_mask}
        ...
        return self._mask_clip_forward(image, attn_mask, num_mask_token), patch_mask
    ...
    def get_mask_embed(self, image, mask):
        ...
        # [B, Q, C]
        mask_embed, patch_mask = self.encode_image_with_mask(image, mask)
        # print(f"mask embed shape: {mask_embed.shape}")

        return mask_embed, patch_mask
    ...
    def pred_logits(self, mask_embed, text_embed, labels):
        ...
        # print(f"CLIP logit scale: {self.logit_scale}")

        logit_per_mask = ensemble_logits_with_labels(logit_per_mask, labels)

        return logit_per_mask
    
    def pred_logits2(self, mask_embed, text_embed, labels):
        logit_per_mask = (
            torch.einsum(
                "bqc,nc->bqn", F.normalize(mask_embed, dim=-1), F.normalize(text_embed, dim=-1)
            )
    ...
    def get_mask_embeddings(self, image, mask):
        mask_embed, patch_mask = self.get_mask_embed(image, mask)
        output = {"mask_embed": mask_embed} | {"patch_mask" : patch_mask}
