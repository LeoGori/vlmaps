...
class CategoryODISE(MaskFormer):
    def __init__(
        self,
        *,
        category_head=None,
...
#         # [B, Q, C]
#         mask_embed = outputs["mask_embed"]
#         # print(f"mask embedding shape: {mask_embed.shape}")
#         # [K, C]
#         text_embed = outputs["text_embed"]
...
#         # print(f"mask2former logit scale: {logit_scale}")

#         # print(logit_scale.is_cuda)
#         # print(text_embed.is_cuda)
#         # print(logit_scale.is_cuda)
...
#         # print(f"{pred.shape} before ensemble_logits_with_labels")

#         pred = ensemble_logits_with_labels(pred, labels, ensemble_method="max")

#         # print(f"{pred.shape} after ensemble_logits_with_labels")

#         return pred
    
#     def cal_pred_open_logits2(self, mask_embed, text_embed, logit_scale):
...
#         features, context = self.backbone(images.tensor)
#         outputs = self.sem_seg_head(features)
...
#             mask_foreground_logits = copy.deepcopy(mask_cls_results)
...
#                 [masks_class_probs * binary_probs[..., 0:1], binary_probs[..., 1:2]], dim=-1
#             )
#             # print(f"masks class probs: {masks_class_probs.shape}")
#             # print(f"binary probs: {binary_probs.shape}")
#             # NOTE: mask_cls_results is already multiplied with logit_scale,
#             # avoid double scale, which cause overflow in softmax
#             # mask_cls_results = torch.log(mask_cls_results + 1e-8) * outputs["logit_scale"]
#             mask_cls_results = torch.log(mask_cls_results + 1e-8)
...
#             clip_results = outputs["clip_results"]
#             projected_mask_embed = outputs["mask_embed"]
#             logit_scale = outputs["logit_scale"]
#             del outputs

#             # print(f"mask_cls_results: {mask_cls_results.shape}")
#             # print(f"mask_pred_results: {mask_pred_results.shape}")
...
#             processed_results[-1]["clip_results"] = clip_results
#             processed_results[-1]["mask_pred"] = mask_pred_result
#             processed_results[-1]["projected_mask_embed"] = projected_mask_embed
#             processed_results[-1]["logit_scale"] = logit_scale
#             processed_results[-1]["word_embedder"] = self.word_head
#             processed_results[-1]["captions"] = context
#             processed_results[-1]["mask_foreground_logits"] = mask_foreground_logits

#             return processed_results
        

# class CaptionODISE(ODISE):
class CaptionODISE(MaskFormer):
    def __init__(
        self,
        *,
        word_head=None,
        ...
        **kwargs,
        ...
        self.other_embedding = self.word_head.get_and_cache_test_text_embed(labels=["other"])
    ...
    def __preprocess_input(self, original_image):
    ...
    def get_image_embeddings(self, image):
        ...
        batched_inputs = self.__preprocess_input(image)
        ...
        self.maskformer_logit_scale = outputs["logit_scale"]

        if self.clip_head is not None:
            outputs.update(self.clip_head.get_masks_embeddings(outputs))

        assert mask_cls_results.shape[-1] == 2

        original_image_height = batched_inputs[0]['height']
        original_image_width = batched_inputs[0]['width']
        ...
        if self.sem_seg_postprocess_before_inference:
            mask_pred_results = retry_if_cuda_oom(sem_seg_postprocess)(
                mask_pred_results, image_size, original_image_height, original_image_width
            )

        # mask_pred_results = F.interpolate(
        #     mask_pred_results, size=(original_image_height, original_image_width), mode="bilinear", align_corners=False
        #     )

        # order masks by size in increasing order
        sort_index = self.__order_masks_by_size(mask_pred_results)

        scores, labels = F.softmax(mask_cls_results, dim=-1).max(-1)
        ...
        mask_embeds = outputs["clip_results"]["mask_embed"].squeeze()
        projected_mask_embeds = outputs["mask_embed"].detach().squeeze()
        projected_clip_embeds = self.word_head.text_proj(mask_embeds).detach()

        # TODO: 768 is CLIP embedding size, while 256 is Mask2Former embedding size (change from hardcoded to parametric)
        featured_image = torch.zeros(original_image_height, original_image_width, 768 + 256).cuda()
        ...
        for i in sort_index:
            ...
            masked_image = mask_pred_results[i].sigmoid_() < 0.5
            masked_image = masked_image.squeeze().unsqueeze(-1).expand(feat_shape)

            assert masked_image.shape[:2] == (original_image_height, original_image_width), masked_image.shape
        ...
        # Check non-zero elements in one go, these are the non filled pixels of the image, which needs to be replaced by something
        # In this implementation they are replaced with the "other" embedding
        non_empty_count = torch.count_nonzero(featured_image)
        full_count = original_image_height * original_image_width * featured_image.shape[-1]
        if non_empty_count < full_count:
            # print(non_empty_count / full_count)
            # print(self.other_embedding.is_cuda)
            # print(self.projected_other_embedding.is_cuda)
            featured_image = torch.where(featured_image == 0, torch.cat((self.other_embedding.squeeze().cuda(), self.projected_other_embedding.squeeze().cuda())), featured_image)
    ...
    def get_segmentation(self, category_list, featured_voxel):
        ...
        embeddings = self.word_head.get_and_cache_test_text_embed(labels=word_list)

        projected_embeddings = self.word_head.text_proj(embeddings).detach()

        category_overlapping_list = []

        for test_label in category_list:
            category_overlapping_list.append(not set([l for label in self.clip_head.train_labels for l in label]).isdisjoint(set(test_label)))
        ...
        # TODO: remove fixxed size CLIP 
        clip_embeddings = featured_voxel[:, :, :768]
        mask2former_embeddings = featured_voxel[:, :, 768:]

        # Batch compute logits for all pixels
        mask2former_logits = self.cal_pred_open_logits2(mask2former_embeddings, projected_embeddings, self.maskformer_logit_scale)

        # Compute CLIP logits in batch
        clip_logits = self.clip_head.clip.pred_logits(clip_embeddings, embeddings, word_list).squeeze(-1)
        ...
        pred_open_logits_base = (
            (pred_open_prob ** (1 - self.clip_head.alpha) * mask_pred_open_prob**self.clip_head.alpha).log()
            # * outputs["logit_scale"]
            * category_overlapping_mask
        )

        pred_open_logits_novel = (
            (pred_open_prob ** (1 - self.clip_head.beta) * mask_pred_open_prob**self.clip_head.beta).log()
            # * outputs["logit_scale"]
...
class PoolingCLIPHead(WordEmbed):
    ...
    def get_masks_embeddings(self, outputs):
        
        assert not self.training, "PoolingCLIPHead only supports inference"

        mask_pred_results = outputs["pred_masks"]

        clip_results = self.clip.get_mask_embeddings(
            outputs["images"],
            mask_pred_results
        )

        ret = {"clip_results": clip_results}
    ...
    def forward(self, outputs, targets=None):
        ...
        # print([l[0] in train_labels for l in self.test_labels])
        ...
        # print(category_overlapping_list)
        # print(category_overlapping_mask)
